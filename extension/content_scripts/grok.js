// content_scripts/grok.js — Grok Imagine adapter (classic content script)
(function () {
  if (window.__bulkStudioGrokLoaded) return;
  window.__bulkStudioGrokLoaded = true;
  ...
  // Acquire latest selectors from storage
  ...
  async function tryNativeBulk(items, opts) {
    // Strategy: locate a multi-line prompt area or "Run All" workflow.
    ...
    // Fallback: sequential single submissions in the same tab (fast queue)
  ...
  async function scanResultCards() {
    ...
    for (const el of candidates) {
      ...
      // Check completed (video has duration > 0, image is loaded)
      const ready = isVideo
  ...
  function detectBlocks() {
    ...
    if (/log\s*in|sign\s*in/i.test(text) && /please/i.test(text)) return "login_required";
    ...
    (async () => {
      try {
        ...
        if (t === "RUN_BATCH") {
          ...
          // Watch for layout change for next 30s
  ...
  // Periodically report card updates to the background
  let polling = null;
